 ...
 * occurs:
 * 当Java虚拟机启动时，通常只有一个非守护进程线程（它通常调用某个指定类的名为<code>main</code>的方法）。
 * Java虚拟机将继续执行线程，直到发生以下任一情况
 * <ul>
 * <li>The exit method of class Runtime has been
 *     called and the security manager has permitted the exit operation
 *     to take place.
 *     类运行时的exit方法已被调用，安全管理器已允许执行退出操作
 ...
 * <p>
 * 除非另有说明，否则向此类中的构造函数或方法传递{@code null}参数将导致抛出{@link NullPointerException}
...
class Thread implements Runnable {
     ...
     * @param name the name of the new Thread 线程的名称
     * @param stackSize the desired stack size for the new thread, or 新线程所需的堆栈大小，或
     *        zero to indicate that this parameter is to be ignored. 0表示忽略此参数
