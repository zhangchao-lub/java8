 ...
 * 双向列表实现了list和deque接口。实现了所有可选的列表操作并允许空值
 * <p>All of the operations perform as could be expected for a doubly-linked
 * list.  Operations that index into the list will traverse the list from
 * the beginning or the end, whichever is closer to the specified index.
 * 对于双链接列表，所有操作的执行都符合预期。索引到列表中的操作将从开始或结束遍历列表，以更接近指定索引为准
 *
 ...
 * 注意，这个实现是不同步的。如果多个线程同时访问一个链表，并且至少有一个线程在结构上修改了该链表，则它必须在外部同步。
 * （结构修改是添加或删除一个或多个元素的任何操作；仅仅设置元素的值不是结构修改）
 * 这通常是通过在一些自然封装列表的对象上进行同步来实现的
 *
 ...
 * 如果不存在这样的对象，则应该使用Collections的synchronizedList方法“包装”该列表。最好在创建时执行此操作，以防止意外的对列表的非同步访问
 *
 ...
 * 以任何方式（除了通过迭代器自己的remove或add方法）对列表进行结构修改，迭代器将抛出一个ConcurrentModificationException。
 * 因此，在并发修改的情况下，迭代器会迅速而彻底地失败，而不是在将来某个不确定的时间通过未知的行为而产生风险
 *
 ...
 * 注意，迭代器的fail-fast行为不能得到保证，因为一般来说，在存在不同步的并发修改时，不可能做出任何明确的保证。
 * Fail fast迭代器尽最大努力抛出{@code ConcurrentModificationException}。
 * 因此，编写一个依赖这个异常来保证其正确性的程序是错误的：
 * 迭代器的fail-fast行为应该只用于检测bug
 *
