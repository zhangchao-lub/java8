...
public class HashMap<K,V> extends AbstractMap<K,V>
   ...
   //容量最小64时才会转会成红黑树
    ...
    final Node<K,V> getNode(int hash, Object key) {
        ...
        // 数组不为null && 数组长度大于0 && hash算出来的索引位首节点不为null
     ...
     */
    //1:检查哈希表是否为空，若为空，则进行初始化。
    //2:通过key的hash值以及哈希表长度来确定当前key在哈希表中的索引下标，并获取到同一下标下的链表首节点或者红黑树的根节点
    // 如果获取到首节点（或根节点）为null，说明当前哈希表的位置上没有任何链表或者红黑树，此时将key和value封装成Node节点对象存储到首节点位置
    //3:如果获取到首节点（或根节点）不为null，说明当前哈希表的位置上有链表或者红黑树，这是进一步判断当前key是否和首节点的Node中key一致，
    // 如果一致，则将首节点的值替换成新值，否则进行下一步
    //4:如果当前需要存储的key和首节点的key不一致，那么进一步判断当前首节点是否为TreeNode类型，也就是红黑树类型，如果是红黑树类型，
    // 则做树节点插入，否则进行下一步
    //5:如果是链表，递归循环
    //6:链表中的元素的key有和入参key相等的，允许覆盖值的话直接覆盖
    //put方法默认覆盖
    //7:如果新增的元素在链表中不存在，则新增，新增到链表的尾部
    //8:新增时，判断如果链表的长度大于等于8时，“尝试”转红黑树
    //9:上面之所以说是尝试转换成红黑树，这是因为在转换逻辑里面对哈希表的长度还进行了校验，只有哈希表长度大于等于64才转换，否则进行扩容
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        // 通过对hash与数组长度的与操作，确定key对应的数组位置，获取指定key的对应下标的首节点并赋值给p
        // 如果首节点为null，那么创建一个新的Node节点作为首节点
        ...
        // 如果hash冲突，当前索引上有值
            ...
            // e 当前节点的临时变量
            Node<K,V> e; K k;
            // 如果 key 的 hash 和值都相等，那么这存放的两个元素是相同的，直接把当前下标位置的 Node 值赋值给临时变量e
            if (p.hash == hash &&
            ...
            if (e != null) {
                V oldValue = e.value;
                // 当 onlyIfAbsent 为 false 或者原值为null时，才会覆盖值
    ...
    // 扩容是双倍扩容，要么还在原来索引位置
    // 要么在2次幂的地方 move with a power of two offset in the new table (不知道如何翻译)
